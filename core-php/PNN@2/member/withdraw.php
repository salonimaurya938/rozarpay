...
$wwaa=$mysqli->query("SELECT wcrg FROM general_setting WHERE id='1'");
$wCharge=$wwaa->fetch_array(MYSQLI_BOTH);
...
$frg=$mysqli->query("SELECT trxpin FROM users WHERE username='".$user."'");
$tpin=$frg->fetch_array(MYSQLI_BOTH);
...
$res = $mysqli->query("INSERT INTO wd SET wdid='".$wdid."', usid='".$uid."', amount='".$amount."', wcrg='".$wcrg."', method='".$method."', details='".$details."', tm='".time()."', status='0'");
...
$mysqli->query("UPDATE users SET mallu='".$willupdate."' WHERE mid='".$uid."'");
//////////---------------------------------------->>>> CUT THE BALANCE
...
////------------------------------>>>>>>>>>TRX
$mysqli->query("INSERT INTO trx SET usid='".$uid."', trx='".$txn_id."', tm='".time()."', description='".$des."', amount='-".$amount."', nbal='".$willupdate."', acctyp='1'");
////------------------------------>>>>>>>>>TRX
...
$i = 1;
$ddaa = $mysqli->query("SELECT wdid, amount, method, details, tm, status, ptm, wcrg FROM wd WHERE usid='".$uid."' ORDER BY id DESC");
