<?php 
$pgtitle = "Product Cashback History";
...
<h1>Product   <span class="shop-green">Cashback</span></h1>
</div><!--/end container-->
</div>
<!--=== End Breadcrumbs v4 ===-->
<!--=== Registre ===-->
...
<span class="caption-subject bold uppercase"> Product Cashback</span>
</div>
<div class="tool"> </div>
</div>
...
$i = 1;
$ddaa = $mysqli->query("SELECT id,name,email,pone,address,pmode,amount_paid,products,time_date,username, status,cmordptm,prduct_trx,cbkstatus,cashback  FROM orders WHERE username='".$user."' AND status='0'OR status='1' OR status='2' ORDER BY id DESC");
...
if($data[13]==0){
    ...
    $ptm = date("d-m-Y h:i:s A", $data[11]);
}elseif($data[13]==2){
