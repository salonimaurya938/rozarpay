...
$ttop=$mysqli->query("SELECT COUNT(*) FROM users WHERE email='".$email."'");
$eee=$ttop->fetch_array(MYSQLI_BOTH);
if($eee[0]!=0){
//$err7=1;
}
...
$mmg=$mysqli->query("SELECT COUNT(*) FROM users WHERE mobile='".$mobile."'");
$mob=$mmg->fetch_array(MYSQLI_BOTH);	
if($mob[0]!=0){
//$err9=1;
}
...
$res = $mysqli->query("UPDATE users SET fname='".$firstname."', lname='".$lastname."', address='".$address."', city='".$city."', postcode='".$post."', state='".$country."', mobile='".$mobile."', email='".$email."' WHERE mid='".$uid."'");
...
$ggvn=$mysqli->query("SELECT fname, lname, email, mobile, address, city, postcode, state FROM users WHERE mid='".$uid."'");
...
?>
		...
		<section>
		<label class="input">
		<input type="text" name="address" placeholder="Village Address" value="<?php echo $old[4]; ?>" class="form-control">
		</label>
		</section>
		</div>
		<div class="col-sm-6">
		<section>
		<label class="input">
		<input type="text" name="city" placeholder="City" value="<?php echo $old[5]; ?>" class="form-control">
		</label>
		</section>
		</div>
		</div>
		...
		<section>
		<label class="input">
		<input type="text" name="country" placeholder="State" value="<?php echo $old[7]; ?>" class="form-control">
		</label>
		</section>
		</div>
		</div>
